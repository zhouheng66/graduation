      ...
      <main>
        ...
        <div class="outbag">
          <div class="form">
            ...
            <!-- 登录 提交按钮信息 -->
            <div class="submit" @click="uesrLogin">
              <span> <a href="javaScript:"> 登 录</a> </span>
            </div>
    ...
    async uesrLogin() {
      ...
      try {
        if (this.userInfo.name != "" || this.userInfo.password != "") {
          let result = await this.$API.userLogin(this.userInfo);
          if (result.code == 200) {
            let token = result.token;
            this.$message({ message: "用户登录成功", type: "success" });
            localStorage.setItem("token", token); // 将token存储在本地存储中
            this.$router.push({ name: "mainPage" });
          }
        }else{
        this.$message({ message: "用户名密码不能为空", type: "warning" });

        }
      } catch (error) {
        this.$message({ message: "用户名密码不正确", type: "warning" });
      }
    },
  },
};
